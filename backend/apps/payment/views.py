...
class ProcessPaymentView(APIView):
    def post(self, request, format=None):
        ...
        # review if data shipping is valid
        if not Shipping.objects.filter(id__iexact=shipping_id).exists():
            return Response(
                {'error': 'Invalid shipping options'},
                status=status.HTTP_404_NOT_FOUND
            )

        cart = Cart.objects.get(user=user)

        # review if user has items in the cart
        if not CartItem.objects.filter(cart=cart).exists():
            return Response(
                {'error': 'Need to have items in cart'},
                status=status.HTTP_404_NOT_FOUND
        ...
        for cart_item in cart_items:
            ...
            if new_transaction.is_success or new_transaction.transaction:
                for cart_item in cart_items:
                    ...
                    # find product quantity after buy
                    quantity = int(update_product.quantity) - int(cart_item.count)

                    # find product quantity to sale
                    sold = int(update_product.sold) - int(cart_item.count)

                    # update product
                    Product.objects.filter(id=cart_item.product.id).update(quantity=quantity, sold=sold)
                ...
                try:
                    send_mail(
                        ...
                        + '\n\nSincerely,'
                        + '\nShop Time',
                        'luizthiago20sccp@gmail.com',
                        [user.email],
                        fail_silently=False
                    )
                except:
                    return Response(
                        {'error': 'Transaction succeeded and order created, but failed to to send email'},
                        status=status.HTTP_500_INTERNAL_SERVER_ERROR
                    )
